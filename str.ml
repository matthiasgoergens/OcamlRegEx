...
type simple_syntax =
    | Literal of char
    | Alternate of simple_syntax list
    | Catenate of simple_syntax list
    | Empty
...
let rec compile : simple_syntax -> automaton =
  function
    ...
    | Catenate s ->
        ...
        ; fin = (List.hd (List.rev automata)).fin
        ; table = List.concat
                (List.map (fun (a,b) -> (a.fin, None, b.start)) (neighbours automata)
                :: tables)}
    | Kleene s ->
        let auto = compile s in
        { start = auto.start; fin = auto.fin
        ; table = (auto.fin, None, auto.start)::auto.table}
